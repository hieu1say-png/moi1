 ...
 *
 * GEOMETRY LAB - CENTRAL MATHEMATICS ENGINE (SERVER-SIDE)
 */
...
export class MathEngine {
  ...
  /**
   * Diagnoses common student formula and numerical errors.
   */
  public static diagnose(
    shape: 'CYLINDER' | 'CONE' | 'SPHERE' | 'PARADOX_1_3' | 'GENERAL_GEOMETRY',
    query: string,
    contextValues: { r?: number | null; h?: number | null; l?: number | null; expectedAnswer?: any }
  ): MathDiagnosisResult {
    const clean = query.toLowerCase();

    // 1. Formula Error: Missing square (e.g. \pi r h or πrh for cylinder volume)
    if ((clean.includes('πrh') || clean.includes('pirh') || clean.includes('\\pi rh')) && !clean.includes('2πrh')) {
      return {
        detectedError: 'MISSING_SQUARE',
        errorStepExplanation: {
          step: 'Bước lập công thức thể tích',
          reason: 'Em đã quên bình phương bán kính đáy ($r$ thay vì $r^2$). Công thức đúng của thể tích là diện tích đáy nhân chiều cao: $S_{\\text{đáy}} = \\pi r^2 \\implies V = \\pi r^2 h$.',
          fix: 'Thay $r$ bằng $r^2$: $V = \\pi r^2 h$.'
        }
      };
    }

    // 2. Formula Error: Cone forgot 1/3
    if (shape === 'CONE' && clean.includes('πr^2h') && !clean.includes('1/3') && !clean.includes('\\frac{1}{3}')) {
      return {
        detectedError: 'FORGOT_ONE_THIRD',
        errorStepExplanation: {
          step: 'Bước chọn công thức thể tích hình nón',
          reason: 'Công thức $\\pi r^2 h$ là thể tích của hình trụ. Thể tích hình nón chỉ bằng đúng $\\frac{1}{3}$ thể tích hình trụ cùng đáy và chiều cao.',
          fix: 'Bổ sung hệ số $\\frac{1}{3}$: $V_{\\text{nón}} = \\frac{1}{3}\\pi r^2 h$.'
        }
      };
    }

    // 3. Formula Error: Cone confused h with l in Sxq or V
    if (shape === 'CONE' && (clean.includes('πrh') || clean.includes('\\pi rh'))) {
      return {
        detectedError: 'GENERATRIX_HEIGHT_CONFUSION',
        errorStepExplanation: {
          step: 'Bước tính diện tích xung quanh hình nón',
          reason: 'Diện tích xung quanh hình nón sử dụng đường sinh $l$ chứ không dùng chiều cao $h$.',
          fix: 'Dùng công thức $S_{xq} = \\pi r l$ với $l = \\sqrt{h^2 + r^2}$.'
        }
      };
    }

    // 4. Numerical Check Answer if student provided answer like "128pi"
    const r = contextValues.r ?? 4;
    const h = contextValues.h ?? 8;

    if (shape === 'CYLINDER' && r && h) {
      const exactVPi = r * r * h; // 4^2 * 8 = 128
      if (clean.includes(`${exactVPi}pi`) || clean.includes(`${exactVPi}\\pi`) || clean.includes(`${exactVPi} π`)) {
        return {
          isCorrect: true,
          expectedPiMultiple: exactVPi,
          expectedExactLatex: `$V = ${exactVPi}\\pi$`
        };
      }
    }
