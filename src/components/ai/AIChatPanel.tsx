  ...
  return (
    ...
    >
      ...
      <div className="px-3 py-2 bg-gray-50 border-t border-gray-200 space-y-1">
        <div className="flex items-center justify-between text-[10px] font-semibold text-gray-500 uppercase tracking-wider px-1">
          <span>Gợi ý câu hỏi nhanh:</span>
          <span className="text-[9px] text-gray-400 font-normal">Nhấn để điền và chỉnh sửa</span>
        </div>
        <div className="flex gap-1.5 overflow-x-auto pb-1 no-scrollbar">
          {(dynamicQuickReplies.length > 0 ? dynamicQuickReplies.slice(0, 4) : quickActionChips).map((chip, idx) => (
            <button
              ...
              disabled={isSending}
              className="px-2.5 py-1 rounded-xl bg-white hover:bg-orange-50 text-gray-700 hover:text-orange-700 border border-gray-200 hover:border-orange-300 text-xs font-medium transition-colors cursor-pointer shrink-0 whitespace-nowrap shadow-2xs active:scale-95"
            >
              {chip.label}
            </button>
          ))}
      ...
      {/* ---------------------------------------------------- */}
      {/* 5. INPUT AREA (<textarea>, Enter, Shift+Enter, [GỬI]) */}
      {/* ---------------------------------------------------- */}
        ...
        <div className="flex items-end gap-2">
          <div className="flex-1 relative">
            <textarea
              ...
              className="w-full text-xs sm:text-sm px-3.5 py-2.5 rounded-xl border border-gray-300 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 resize-none outline-none transition-all leading-relaxed bg-white text-gray-900 placeholder:text-gray-400 disabled:bg-gray-100 disabled:text-gray-500 min-h-[52px]"
            />
          </div>
          ...
          <button
            ...
            className="px-4 py-3 bg-orange-500 hover:bg-orange-600 disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed text-white rounded-xl font-bold text-xs sm:text-sm transition-all cursor-pointer flex items-center justify-center gap-1.5 shadow-xs shrink-0 min-h-[52px] min-w-[70px] active:scale-95"
          >
            <span>GỬI</span>
